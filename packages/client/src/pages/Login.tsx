...
export default function Login() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const { ok, status, data } = await apiPost('/auth/login', { username, password })
    ...
    <div className="min-h-screen flex items-start justify-center pt-[calc(50vh-12rem)] px-4">
      <Card className="w-full max-w-sm bg-zinc-900 border-zinc-800">
        <CardContent className="flex flex-col gap-4 p-6">
          <h1 className="text-xl font-semibold leading-[1.2] text-zinc-50">Sign in</h1>
          ...
          <p className="text-sm text-zinc-400 text-center">
            No account?{' '}
            <Link to="/register" className="underline text-zinc-400 hover:text-zinc-50">
              Create one
            </Link>
          </p>
        </CardContent>
